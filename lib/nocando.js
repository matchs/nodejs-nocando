    ...
    return {
        can : function(action, context, authorization, resource_provider, authorized_resource_provider){
            return new AuthMonad(
                saveAuthorization(this.value, action, context, authorization,
         ...
         *@return MaybeMonad
         */
        authorize : function(state, action, target, user, context){
              return new MaybeMonad(can_i(this.value, action, target, user, context));
        }
    };
};

var makeKey = function(context, action){
    return context + '-' + action;
};

var equals = function(a, b){
    var keyA = makeKey(a.action, a.context);
    var keyB = makeKey(b.action, b.context);
    return keyA === keyB ||
           makeKey(a.context, '*') === keyB;
};
...
var saveAuthorization = function(list, action, context, authorization, authorization_resource_provider, authorized_resource_provider){
    var key = makeKey(context, action);
...
var can = function(action, context, authorization, resource_provider, authorized_resource_provider){
    ...
    return new AuthMonad(
        saveAuthorization(fastMap, action, context,
...
var can_i = function(list, action, target, user, context){
    ...
    if(auth){
        var resource = auth.brp(user, context);
        if(auth.auth(user, context, resource)){
            return auth.arp(user, context, resource);
        } else {
            return false;
        }
    } else {
